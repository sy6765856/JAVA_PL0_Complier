...
public class Parsing implements ByteCode
{
	...
	public Parsing(LexiAnalyse CF,String file_name)
	{
		Lexi = CF;
		word = Lexi.GetToken();
        try{Writer wt=new FileWriter(new File(file_name+".java"));}catch (IOException e){}
		analyse(file_name,wt);
        try{wt.close();}catch (IOException e){}
	}
	...
	public void analyse(String file_name,Writer wt) {
        String str="public class "+file_name+"{";
        try{wt.write(str);}catch (IOException e){}
		int tx0;
		lev++;
		dx = 3;
		tx0 = nameArray.size();
				...
				|| word.getSym().equals("procsym"))
        {
			...
			{
                str="public static int ";
                try{wt.write(str);}catch (IOException e){}
                
				word = Lexi.GetToken();
                try{wt.write(word.getNam());}catch (IOException e){}
                
				delcaration(wt);
				while (word.getSym().equals("comma"))
				{
                    try{wt.write(",");}catch (IOException e){}
					word = Lexi.GetToken();
                    try{wt.write(word.getNam());}catch (IOException e){}
					delcaration(wt);
				}
	...
	public void delcaration(Writer wt)
	{
		if (word.getSym().equals("ident"))
		{
			id = word.getNam();
	...
	public void vardeclaration(Writer wt)
	{
		if (word.getSym().equals("ident")) {
			id = word.getNam();
			enter("variable");
